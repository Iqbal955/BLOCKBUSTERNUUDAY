...
useEffect(() => {
    fetch(
      `https://feed.entertainment.tv.theplatform.eu/f/jGxigC/bb-all-pas?form=json&lang=da&byProgramType=movie&range=1-20`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleLoadMore = async () => {
    ...
    const req = await fetch(
      `
      https://feed.entertainment.tv.theplatform.eu/f/jGxigC/bb-all-pas?form=json&lang=da&byProgramType=movie&range=${
        1 - (range.prev + 20)
      }`
    );
      ...
      {loading ? (
        ...
        <>
          <ContentHeader>
            ...
            <h6>Movies Count: {genreLength}</h6>
          </ContentHeader>
          {queryGenre?.length === 0 ? (
